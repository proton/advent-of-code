...
pub fn main() {
  if let Ok(lines) = read_lines("input.txt") {
    ...
    let custom_packet1 = parse_line(&"[[2]]".to_string());
    let custom_packet2 = parse_line(&"[[6]]".to_string());
    packets.push(custom_packet1.clone());
    packets.push(custom_packet2.clone());
    ...
    packets.sort_by(|a, b| i32_to_ordering(compare(a.clone(), b.clone())) );
    
    let index1 = packets.iter().position(|x| x == &custom_packet1).unwrap() + 1;
    let index2 = packets.iter().position(|x| x == &custom_packet2).unwrap() + 1;
...
fn i32_to_ordering(n: i32) -> cmp::Ordering {
    if n < 0 { return cmp::Ordering::Less; }
    if n > 0 { return cmp::Ordering::Greater; }
