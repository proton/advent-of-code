...
pub fn main() {
  if let Ok(lines) = read_lines("input.txt") {
    ...
    let need_free_size    = 30000000;
    let current_free_size = total_disk_size - dir_sizes.get("//").unwrap();

    let total: usize = *dir_sizes.values().filter( |size| current_free_size + **size >= need_free_size ).min().unwrap();
    println!("{}", total);
  }
}
